package no.novari.fint.consumer.models.rolle;
...
public class RolleController {
    ...
    public RolleResources getRolle(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getRolleByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Rolle cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public RolleResources getRolleByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getRolle(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private RolleResources getRolleByOdataFilter(
        ...
        Event event = new Event(
                ...
                FullmaktActions.GET_ROLLE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new RolleResources();
    
        ArrayList<RolleResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<RolleResource>>() {});
    ...
    @GetMapping("/navn/{id:.+}")
    public RolleResource getRolleByNavn(
        ...
        log.debug("navn: {}, OrgId: {}, Client: {}", id, orgId, client);

        Event event = new Event(orgId, Constants.COMPONENT, FullmaktActions.GET_ROLLE, client);
        event.setOperation(Operation.READ);
        event.setQuery("navn/" + id);

        if (cacheService != null) {
            fintAuditService.audit(event);
            fintAuditService.audit(event, Status.CACHE);

            Optional<RolleResource> rolle = cacheService.getRolleByNavn(orgId, id);
